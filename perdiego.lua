...
startBatAimbot = function()
	...
	Conns.aimbot = RunService.RenderStepped:Connect(function()
		...
		local chaseSpeed = State.autoBatSpeed
...
for _, name in pairs({"AxisHubGUI"}) do
...
-- ============================================================
-- COLORS (RED NEON THEME - AXIS VS STYLE)
-- ============================================================
local _C = {
	[1] = Color3.fromRGB(25, 8, 12),     -- BG Principale (Rosso scuro)
	[2] = Color3.fromRGB(12, 4, 8),      -- Sidebar (Rosso molto scuro)
	[3] = Color3.fromRGB(35, 10, 15),    -- Card (Rosso scuro)
	[4] = Color3.fromRGB(55, 15, 25),    -- Card Hover
	[5] = Color3.fromRGB(255, 60, 70),   -- Border principale (Neon Red)
	[6] = Color3.fromRGB(200, 40, 55),   -- Border secondario
	[7] = Color3.fromRGB(255, 245, 245), -- Text Bianco
	[8] = Color3.fromRGB(255, 160, 170), -- DIM
	[9] = Color3.fromRGB(200, 100, 120), -- DIM2
	[10]= Color3.fromRGB(45, 10, 18),    -- Button / KB (Rosso scuro)
}
...
-- Colori Extra (Red Theme)
local TOPBAR_BG   = Color3.fromRGB(18, 6, 10)
local PORTRAIT_BG = Color3.fromRGB(22, 8, 14)
local MINI_BG     = Color3.fromRGB(18, 6, 10)
local IDLE_TAB_BG = Color3.fromRGB(28, 10, 16)
local ACTIVE_TAB_BG = Color3.fromRGB(70, 20, 30)

-- ============================================================
-- FONT ACCATTIVANTI E LEGGIBILI
-- ============================================================
local FONT_MAIN   = Enum.Font.GothamBold       -- Bold e chiaro
local FONT_TITLE  = Enum.Font.Bangers          -- Titoli forti
local FONT_NORMAL = Enum.Font.Gotham

local W, H, SW = 340, 370, 88 
local PW = 0         
local CORNER = 12
...
titleLbl.Text = "AXIS.VS"
titleLbl.TextColor3 = WHITE
titleLbl.Font = FONT_TITLE
titleLbl.TextSize = 15.5
...
verLbl.Text = "discord.gg/Bj4kvhhDjh"
verLbl.TextColor3 = DIM
verLbl.Font = FONT_MAIN
verLbl.TextSize = 9
...
minBtn.MouseLeave:Connect(function() TweenService:Create(minBtn, TweenInfo.new(0.1), {BackgroundColor3=TOPBAR_BG}):Play() end)
...
content.Size = UDim2.new(1, -(SW + 2), 1, -44 - CORNER)
...
mini.Name = "AxisMini"
mini.Size = UDim2.new(0, 180, 0, 36)        -- più grande
mini.Position = UDim2.new(0, 50, 0, 50)
mini.BackgroundColor3 = MINI_BG
mini.BorderSizePixel = 0
mini.Text = "AXIS.VS"
mini.TextColor3 = WHITE
mini.Font = FONT_TITLE
mini.TextSize = 15.5                   
...
lbl.Font = FONT_TITLE      -- Cambiato in TITLE (più forte)
lbl.TextSize = 10.5        -- leggermente più grande
...
local function applyTryhardAnims(char)
    local animate = char:FindFirstChild("Animate")
    if not animate then return end
...
local function makeKB(parent, kbEntry, onChange)
	...
	b.BackgroundColor3 = Color3.fromRGB(45, 10, 18)
	b.BorderSizePixel = 0
...
local function rowToggle(tabName, label, sub, defOn, onToggle)
	...
	cLabel(c, label, 10, 160, 11, WHITE, FONT_MAIN)
	if sub then
		local sl = cLabel(c, sub, 10, 170, 9, DIM, FONT_NORMAL)
...
local function rowToggleKB(tabName, label, sub, kbEntry, defOn, onToggle, onKeyChange)
	local c = baseCard(tabName, sub and 48 or 38)
	cLabel(c, label, 10, 120, 11, WHITE, FONT_MAIN)
	if sub then
		local sl = cLabel(c, sub, 10, 150, 9, DIM, FONT_NORMAL)
...
local function rowInput(tabName, label, sub, default, onChange)
	local c = baseCard(tabName, sub and 48 or 38)
	cLabel(c, label, 10, 130, 11, WHITE, FONT_MAIN)
	if sub then
		local sl = cLabel(c, sub, 10, 160, 9, DIM, FONT_NORMAL)
...
local function rowActionBtn(tabName, label, onClick)
	...
	b.BackgroundColor3 = Color3.fromRGB(45, 10, 18)  -- Rosso scuro
	b.BorderSizePixel = 0
	b.Text = label
	b.TextColor3 = WHITE   -- Bianco
	b.Font = FONT_MAIN
	b.TextSize = 11.5
	...
	b.MouseButton1Click:Connect(function()
		TweenService:Create(b, TweenInfo.new(0.08), {BackgroundColor3 = Color3.fromRGB(255, 70, 80)}):Play()
		task.delay(0.15, function() 
			TweenService:Create(b, TweenInfo.new(0.1), {BackgroundColor3 = Color3.fromRGB(45, 10, 18)}):Play() 
		end)
		if onClick then pcall(onClick) end
	end)
	b.MouseEnter:Connect(function() TweenService:Create(b, TweenInfo.new(0.1), {BackgroundColor3=Color3.fromRGB(70, 20, 30)}):Play() end)
	b.MouseLeave:Connect(function() TweenService:Create(b, TweenInfo.new(0.1), {BackgroundColor3=Color3.fromRGB(45, 10, 18)}):Play() end)
	return b
end
...
do
	...
	cLabel(c, "Mode", 10, 80, 11, WHITE, FONT_MAIN)
	modeValLbl = cLabel(c, "Normal", 88, 80, 10, DIM, FONT_MAIN, Enum.TextXAlignment.Center)
...
do
	local c = baseCard("Speed", 38)
	cLabel(c, "Lagger Mode", 10, 120, 11, WHITE, FONT_MAIN)
...
-- Input per la velocità (come vuoi tu)
aimbotSpeedBox = rowInput("Bat Aimbot", "Aimbot Speed", "(default 58)", 58, function(n)
	local val = tonumber(n)
	if val and val >= 20 and val <= 120 then
		State.autoBatSpeed = val
		task.spawn(saveConfig)
	end
end)

-- Forza sincronizzazione forte della casella Aimbot Speed
task.spawn(function()
    task.wait(0.8)  -- aspetta che l'UI sia completamente caricata
    if aimbotSpeedBox and aimbotSpeedBox.Parent then
        aimbotSpeedBox.Text = tostring(State.autoBatSpeed or 58)
    end
end)

setAutoGrab = rowToggle("Mechanics", "Auto Grab", nil, false, function(on)
    Steal.AutoStealEnabled = on
    print("🔧 Auto Grab toggled:", on)  -- DEBUG
    if on then 
        local success = pcall(startAutoSteal)
        print("AutoSteal start result:", success)
    else 
        stopAutoSteal() 
    end
end)
...
-- Tryhard Animation
local setTryhardAnimationToggle = rowToggle("Mechanics", "Tryhard Animation", "Custom tryhard anims", State.tryhardAnimationEnabled, function(on)
    State.tryhardAnimationEnabled = on
    if on then
        startTryhardAnimation()
    else
        stopTryhardAnimation()
    end
end)
...
do
	...
	cLabel(c, "Intro Music", 10, 130, 11, WHITE, FONT_MAIN)
...
do
	...
	resetMobileButtons = function()
		...
		-- Bottoni Mobili
	-- Nella sezione MOBILE BUTTONS (cerca "Q_OFF")
local Q_OFF = Color3.fromRGB(40, 8, 15)      -- Rosso scuro
local Q_ON  = Color3.fromRGB(255, 55, 70)    -- Rosso neon acceso

local Q_TEXT_OFF = Color3.fromRGB(255, 255, 255)  -- Bianco
local Q_TEXT_ON  = Color3.fromRGB(255, 255, 255)  -- Bianco
...
-- ============================================================
-- OPIUM v5.2 LOGIC (message 9) - adapted for Axis Mobile
-- ============================================================
;(function()
...
local CONFIG_FILE="AxisMobileConfig.json"
...
-- Presets
local Presets={}
local PRESET_FILE="AxisMobilePresets.json"; local LAST_PRESET_FILE="AxisMobileLastPreset.json"
...
sg.Name = "AxisStealProgress"
sg.ResetOnSpawn = false
sg.Parent = LP.PlayerGui

local progressContainer = Instance.new("Frame", sg)
progressContainer.Name = "ProgressContainer"
progressContainer.Size = UDim2.new(0, 260, 0, 48)        
progressContainer.Position = UDim2.new(0.5, -130, 1, -80)
progressContainer.BackgroundColor3 = Color3.fromRGB(15, 5, 8)
...
progStroke.Thickness = 2.5
progStroke.Color = Color3.fromRGB(255, 60, 70)

ProgressLabel = Instance.new("TextLabel", progressContainer)
ProgressLabel.Size = UDim2.new(0.4, 0, 0.5, 0)
ProgressLabel.Position = UDim2.new(0, 15, 0, 5)
...
ProgressPercentLabel.Size = UDim2.new(0.55, 0, 0.5, 0)
ProgressPercentLabel.Position = UDim2.new(0.4, 0, 0, 5)
ProgressPercentLabel.BackgroundTransparency = 1
ProgressPercentLabel.Text = ""
ProgressPercentLabel.TextColor3 = Color3.fromRGB(255, 100, 110)
ProgressPercentLabel.Font = Enum.Font.GothamBlack
ProgressPercentLabel.TextSize = 16
ProgressPercentLabel.TextXAlignment = Enum.TextXAlignment.Center

local progTrack = Instance.new("Frame", progressContainer)
progTrack.Size = UDim2.new(0.92, 0, 0, 8)
progTrack.Position = UDim2.new(0.04, 0, 1, -18)
progTrack.BackgroundColor3 = Color3.fromRGB(30, 8, 12)
Instance.new("UICorner", progTrack).CornerRadius = UDim.new(1, 0)

ProgressBarFill = Instance.new("Frame", progTrack)
ProgressBarFill.Size = UDim2.new(0, 0, 1, 0)
ProgressBarFill.BackgroundColor3 = Color3.fromRGB(255, 70, 80)
    ...
    for _, plot in ipairs(plots:GetChildren()) do
        ...
        for _, pod in ipairs(podiums:GetChildren()) do
            pcall(function()
                ...
                local spawn = base and (base:FindFirstChild("Spawn") or base:FindFirstChild("SpawnPoint"))
                if spawn then
...
local function executeSteal(prompt, name)
    ...
    local progressConnection = RunService.Heartbeat:Connect(function()
        ...
        if ProgressPercentLabel then 
            ProgressPercentLabel.Text = getDiscordProgress(math.floor(prog * 100))
        end
    end)
...
local function startAutoSteal()
    ...
    Conns.autoSteal = RunService.Heartbeat:Connect(function()
        ...
        local prompt, name = findNearestPrompt()
        if prompt then 
            print("Found prompt on:", name)
...
saveConfig = function(btn)
    ...
    local cfg = {
        ...
        tryhardAnimationEnabled = State.tryhardAnimationEnabled,
        ...
        panelPos = sp(MobilePanel), mainPos = sp(main), 
        miniPos = sp(mini), pbPos = sp(pbFrame),
    }
    ...
    if ok and enc then
        ...
        if wf then pcall(wf, "AxisMobileConfig.json", enc) end
    end

    if btn then
        local prev = btn.Text
        btn.Text = ok and "✓ Saved!" or "✕ Failed!"
        task.delay(1.5, function() if btn and btn.Parent then btn.Text = prev end end)
    end
end
...
loadConfig=function()
    ...
    if not hasFile then 
        print("[Axis] Nessun config trovato, uso valori di default")
        return 
    end
    local raw; pcall(function() raw=rdf(CONFIG_FILE) end)
    if not raw then 
        print("[Axis] Errore lettura file config") 
        return 
    end
    local cfg; pcall(function() cfg=HttpService:JSONDecode(raw) end)
    if not cfg then 
        print("[Axis] Errore decodifica config") 
        return 
    end

    print("[Axis] Config caricato correttamente")
    ...
    if cfg.laggerCarrySpeed then LS2=cfg.laggerCarrySpeed; task.defer(function() if laggerBox2 then laggerBox2.Text=tostring(LS2) end end) end
          ...
          -- === AUTO BAT SPEED (FIXATO DEFINITIVO) ===
    if cfg.autoBatSpeed and type(cfg.autoBatSpeed) == "number" then
        State.autoBatSpeed = cfg.autoBatSpeed
        print("[Axis] Caricato autoBatSpeed =", State.autoBatSpeed)
    else
        State.autoBatSpeed = 58
        print("[Axis] autoBatSpeed non trovato → default 58")
    end

    -- Aggiornamento FORTE della casella
    task.spawn(function()
        task.wait(1.2)
        if aimbotSpeedBox and aimbotSpeedBox.Parent then 
            aimbotSpeedBox.Text = tostring(State.autoBatSpeed)
            print("[Axis] Casella aggiornata a", State.autoBatSpeed)
        end
    end)
    ...
    if cfg.selectedIntroMusic then 
        ...
        task.defer(function() 
            if getgenv().AxisMusicBtn then 
                getgenv().AxisMusicBtn.Text = "Music " .. State.selectedIntroMusic 
            end 
        end)
    end
    ...
    if cfg.tryhardAnimationEnabled then 
        State.tryhardAnimationEnabled = true
        task.defer(function()
            if setTryhardAnimationToggle then 
                setTryhardAnimationToggle(true) 
            end
            startTryhardAnimation()
        end)
    end
    ...
    task.spawn(function()
        task.wait(0.5)
        local function lp(frame, d) 
            if frame and type(d)=="table" and d.xs~=nil then 
                frame.Position=UDim2.new(d.xs,d.xo,d.ys,d.yo) 
            end 
        end
...
local function setupChar(char)
    ...
    if head then
        local oldBB=head:FindFirstChild("AxisBB"); if oldBB then oldBB:Destroy() end
        local bb=Instance.new("BillboardGui",head); bb.Name="AxisBB"
    ...
    -- Tryhard Animation on respawn
    if State.tryhardAnimationEnabled then 
        task.wait(0.3); 
        startTryhardAnimation() 
    end
...
local function setupOtherPlayerSpeed(player)
    ...
    local function onCharacterAdded(char)
        ...
        local oldBB = head:FindFirstChild("Axis")
        ...
        discordLbl.Position = UDim2.new(0, 0, 0, 26)
        discordLbl.BackgroundTransparency = 1
        discordLbl.Text = "discord.gg/Bj4kvhhDjh"
        discordLbl.TextColor3 = Color3.fromRGB(255, 255, 255)
        discordLbl.Font = Enum.Font.GothamBold
        discordLbl.TextScaled = true
        discordLbl.TextStrokeTransparency = 0.1
...
-- ============================================================
-- RUNTIME LOOPS (message 9 version)
-- ============================================================
...
RunService.RenderStepped:Connect(function()
    ...
    if not State.autoBatToggled and not State.autoLeftEnabled and not State.autoRightEnabled then
        ...
        if laggerPhase == 2 then          -- Lagger Carry
            spd = LS2
        elseif laggerPhase == 1 then      -- Lagger normale
            spd = LS
        elseif State.speedToggled then    -- Carry Speed
            spd = CS
        else                              -- Normal Speed
            spd = NS
        end
...
UIS.InputBegan:Connect(function(inp, gp)
    ...
    elseif kbMatch(KB.AutoLeft, kc) then
        ...
        if State.autoLeftEnabled and State.autoBatToggled then State.autoBatToggled = false; pcall(stopBatAimbot); if autoBatSetVisual then autoBatSetVisual(false) end end
    ...
    elseif kbMatch(KB.AutoRight, kc) then
        ...
        if State.autoRightEnabled and State.autoBatToggled then State.autoBatToggled = false; pcall(stopBatAimbot); if autoBatSetVisual then autoBatSetVisual(false) end end
...
task.delay(1, function() pcall(saveConfig) end)
task.spawn(function()
    while task.wait(8) do pcall(saveConfig) end
end)

LP.AncestryChanged:Connect(function()
    if not LP.Parent then pcall(saveConfig) end
end)

print("[Axis.vs] Loaded!")
...
local function playIntroAnimation()
	...
	local SOUL_LOGO_ASSET_ID = "rbxassetid://103621997135520"
	...
	task.spawn(function()
		pcall(function()
			...
			writefile("AxisHubIntro", game:HttpGet(RandomIntro))
			local flex1 = Instance.new("Sound", gethui())
			flex1.SoundId = getcustomasset("AxisHubIntro")
